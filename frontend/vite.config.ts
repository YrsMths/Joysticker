...
process.env.VITE_APP_TITLE ??= process.env.OVERVIEW_TITLE ?? 'joysticker';
...
const GITHUB_PAGES_REPO = 'Joysticker';
const GITHUB_PAGES_HOSTNAME = 'https://yrsmths.github.io/Joysticker';

// https://vitejs.dev/config/
export default defineConfig(({ command }) => {
  const blogPrerenderRoutes = command === 'build' ? getBlogRoutes() : [];
  const isGithubPagesBuild = process.env.GITHUB_ACTIONS === 'true' || process.env.NODE_ENV === 'production';
  const base = isGithubPagesBuild ? `/${GITHUB_PAGES_REPO}/` : '/';
  ...
  return {
    ...
    plugins: [
      ...
      atoms(),
      Sitemap({
        hostname: GITHUB_PAGES_HOSTNAME,
